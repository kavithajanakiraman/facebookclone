...
import os
from werkzeug.utils import secure_filename # collection of libraries that can be used to create a WSGI (Web Server Gateway Interface)
...
def index():
    if request.method == "POST":
        session.pop('user', None) #It clears any existing user session by calling 
...
@app.before_request # function allows you to access the user information throughout the request handling process. 
def password_check():
    g.user = None
    if 'user' in session:
        g.user = session['user']
...
def home():
    ...
    if g.user:
        ...
        return render_template("home-page.html", user=current_user_email, posts=user_posts, username=username, number=number, image=image_path)
